...
AddEventHandler('esx_spdjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
		...
		if targetItem.count > 0 then
			...
			if targetItem.count < amount then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
					...
					TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeItem', sendToDiscord, sourceXPlayer.source, '^5')

					Citizen.Wait(100)
										
					local sendToDiscord2 = '' .. targetXPlayer.name .. ' ถูกยึด ' .. sourceItem.label .. ' โดย ' .. sourceXPlayer.name .. ' จำนวน ' .. ESX.Math.GroupDigits(amount) .. ''
					TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeItem', sendToDiscord2, targetXPlayer.source, '^7')
				end
			end
		else
			TriggerClientEvent("pNotify:SendNotification", _source, {
	...
	elseif itemType == 'item_money' then
		if amount <= 0 then
			...
			if targetMoney < amount then
				...
				TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeMoney', sendToDiscord, sourceXPlayer.source, '^5')

				Citizen.Wait(100)
									
				local sendToDiscord2 = '' .. targetXPlayer.name .. ' ถูกยึด เงินสด โดย ' .. sourceXPlayer.name .. ' จำนวน $' .. ESX.Math.GroupDigits(amount) .. ''
				TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeMoney', sendToDiscord2, targetXPlayer.source, '^7')
			end
	...
	elseif itemType == 'item_account' then
		if amount <= 0 then
			...
			if targetMoney < amount then
				...
				TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeDirtyMoney', sendToDiscord, sourceXPlayer.source, '^5')

				Citizen.Wait(100)
									
				local sendToDiscord2 = '' .. targetXPlayer.name .. ' ถูกยึด ' .. itemName .. ' โดย ' .. sourceXPlayer.name .. ' จำนวน $' .. ESX.Math.GroupDigits(amount) .. ''
				TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeDirtyMoney', sendToDiscord2, targetXPlayer.source, '^7')
			end
	...
	elseif itemType == 'item_weapon' then
		...
		if amount > 0 then
			...
			TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeWeapon', sendToDiscord, sourceXPlayer.source, '^5')
			...
			TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeWeapon', sendToDiscord2, targetXPlayer.source, '^7')
		else
	...
	elseif itemType == "item_key" then -- MEETA GiveKey
		...
		TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeKey', sendToDiscord, sourceXPlayer.source, '^5')

		Citizen.Wait(100)
							
		local sendToDiscord2 = '' .. targetXPlayer.name .. ' ถูกยึด กุญแจรถ โดย ' .. sourceXPlayer.name .. ' ทะเบียน ' .. itemName .. ''
		TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDSeizeKey', sendToDiscord2, targetXPlayer.source, '^7')
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_spd', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDGetStockItem', sendToDiscord, xPlayer.source, '^3')
			end
		else
			TriggerClientEvent("pNotify:SendNotification", _source, {
				text = '<strong class="red-text">จำนวนที่ไม่ถูกต้อง</strong>',
...
AddEventHandler('esx_spdjob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_spd', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDPutStockItem', sendToDiscord, xPlayer.source, '^2')
		else
			TriggerClientEvent("pNotify:SendNotification", source, {
				text = '<strong class="red-text">จำนวนที่ไม่ถูกต้อง</strong>',
				type = "success",
...
ESX.RegisterServerCallback('esx_spdjob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	if removeWeapon then
		...
		TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDPutArmoryWeapon', sendToDiscord, xPlayer.source, '^2')
	end
...
ESX.RegisterServerCallback('esx_spdjob:removeArmoryWeapon', function(source, cb, weaponName)
	...
	TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDGetArmoryWeapon', sendToDiscord, xPlayer.source, '^3')
end)
...
ESX.RegisterServerCallback('esx_spdjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	...
	if type == 1 then
		if xPlayer.getMoney() >= selectedWeapon.price then
			...
			TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDBuyWeapon', sendToDiscord, xPlayer.source, '^4')
	...
	elseif type == 2 then
		...
		if component then
			if xPlayer.getMoney() >= price then
				...
				TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDBuyWeapon', sendToDiscord, xPlayer.source, '^5')
...
ESX.RegisterServerCallback('esx_spdjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'SPDBuyVehicle', sendToDiscord, xPlayer.source, '^2')
	else
		cb(false)
	end
end)
